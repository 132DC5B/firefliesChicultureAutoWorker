...
此README說明專案結構、關鍵元件、建置與除錯方法，以及擴充與維護的建議。
...
- `config.h`：全域設定（cookie 路徑、API base、開關等）。
...
- `#define ENABLE_CURL_LOG`：啟用 libcurl 詳細日誌（若使用 libcurl）
...
本專案採用 MIT License（見 `LICENSE.txt`）。
